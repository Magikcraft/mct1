...
export default class QuestMCT1Magmarun extends QuestMCT1 {
    ...
    start() {
        ...
        [1, 2, 3, 4, 5].forEach(i => {
            const key = `run${i}`
            worldly(world).registerRegion(key, Locs.waypoints[key].region[0], Locs.waypoints[key].region[1]);
        ...
        this.setInterval(() => {
            this.activeRuns.forEach(key => {
                ...
                this.activateRun(Locs.locations[`run${key}`], dispenseType, interval)
            })
        }, (35 * interval))
        ...
        rows.forEach((row, i) => {
            ...
            setTimeout(() => {
                row.forEach((loc, j) => {
                    ...
                    // clean-up
                    setTimeout(() => {
                        const targetLoc = new Location(loc.world, loc.x, loc.y - 11, loc.z)
                        if (dispenseType == 'FIREBALL') {
                            if (j % 4 === 0) {
                                this.getBlocksInRadius(targetLoc, 5, ['FIRE'])
                                    .forEach(block => {
                        ...
                        if (dispenseType == 'ARROW') {
                            if (j % 4 === 0) {
                                this.Locs.world.getNearbyEntities(targetLoc, 5, 10, 5)
                                    .forEach(e => { if (e.type == 'ARROW') e.remove() });
                            }
                        }
                    }, interval * 25)
