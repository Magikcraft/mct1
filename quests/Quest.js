...
var QuestBase = /** @class */ (function() {
    function QuestBase(conf) {
        ...
        this.log = log_1.Logger(
            'mct1/quests/' + this.name + '--' + this.player.name
        )
    ...
    QuestBase.prototype.track = function() {
        ...
        var _a = this,
            ...
            world = _a.world,
            name = _a.name
        var inventoryJSON = user_1
            .user(player)
            .inventory.exportToJSON(
                user_1.user(player).inventory.getAllitemStacks()
            )
        var inventory = inventoryJSON
            .map(function(item, i) {
                return item ? __assign({}, item, { slot: i }) : null
            })
        ...
        var mct1 = user_1.user(player).mct1.isStarted
            ? {
                  ...
                  digestionQueue: user_1
                      .user(player)
                      .mct1.digestionQueue.map(function(item) {
                          return __assign({}, item)
                      }),
                  isStarted: user_1.user(player).mct1.isStarted,
                  isUSA: user_1.user(player).mct1.isUSA,
                  hasInfiniteInsulin: user_1.user(player).mct1
                      .hasInfiniteInsulin,
    ...
    QuestBase.prototype.complete = function() {
        var _a = this,
            player = _a.player,
            options = _a.options,
            nextQuestName = _a.nextQuestName
        this.stop()
    ...
    QuestBase.prototype.setupEndPortal = function() {
        ...
        var _a = this,
            player = _a.player,
            world = _a.world,
            endPortalRegion = _a.endPortalRegion
        if (endPortalRegion) {
            var name = 'endPortal'
            world_1
                .worldly(world)
                .registerRegion(name, endPortalRegion[0], endPortalRegion[1])
            world_1
                .worldly(world)
                .registerPlayerEnterRegionEvent(name, function(event) {
                    _this.complete()
                })
        }
    }
    QuestBase.prototype.setupWayPoints = function() {
        ...
        if (waypoints) {
            var _loop_1 = function(name) {
                ...
                world_1
                    .worldly(world)
                    .registerRegion(key, waypoint.region[0], waypoint.region[1])
                world_1
                    .worldly(world)
                    .registerPlayerEnterRegionEvent(key, function(event) {
...
var QuestMCT1 = /** @class */ (function(_super) {
    ...
    function QuestMCT1(conf) {
        ...
        if (_this.name === 'mct1') _this.name = 'mct1-prologue'
