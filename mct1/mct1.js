...
var MCT1 = /** @class */ (function() {
    function MCT1(player) {
        ...
        this.setSuperSpeed = function(bool) {
            return (_this.hasSuperSpeed = bool)
        }
        this.setSuperJump = function(bool) {
            return (_this.hasSuperJump = bool)
        }
        this.setNightVision = function(bool) {
            return (_this.hasNightVision = bool)
        }
        this.inHealthyRange = function() {
            return _this.bgl >= 4 && _this.bgl <= 8
        }
        ...
        this._playerItemConsume = function(event) {
            ...
            if (event.player.name != _this.player.name) return
            ...
            // Act on POTION drink... (insulin)
                ...
                setTimeout(function() {
                    ...
                    if (_this.hasInfiniteInsulin) _this.ensureInfiniteInsulin()
                }, 1)
                if (_this.debugMode) {
        ...
        this._blockBreak = function(event) {
            ...
            _this.nonMoveActivityLog.push(
                activities_1.activityTypes.BLOCK_BREAK
            )
        }
        this._entityShootBow = function(event) {
            ...
            // Skip if not this.player
            if (event.entity.name != _this.player.name) return
            // Log into nonMoveActivityLog
        ...
        this._projectileLaunch = function(event) {
            ...
            if (!_this.hasLightningSnowballs) return
            var eloc = {
        ...
        this._projectileHit = function(event) {
            ...
            if (event.entity.shooter.name !== _this.player.name) return
            if (!_this.hasLightningSnowballs) return
            if (event.entity.type != 'SNOWBALL') return
            if (event.hitEntity) {
        ...
        this._inventoryClick = function(event) {
            if (event.whoClicked.name !== _this.player.name) return
            ...
            if (
                ...
                if (event.click == 'CREATIVE') {
                    ...
                    if (_this.isInsulinStack(event.currentItem)) {
                        echo(
                            _this.player,
                            'Cannot move insulin while in creative!'
                        )
                        event.setCancelled(true)
                    }
                }
                ...
                event.entity
                    ...
                    .forEach(function(entity) {
                        if (entity.type == 'DROPPED_ITEM' && entity.itemStack) {
                            ...
                            if (
                                _this.isLightningSnowballStack(entity.itemStack)
                            ) {
                                entity.remove()
                            }
    ...
    MCT1.prototype.renderBars = function() {
        ...
        var highGIItems = this.digestionQueue.filter(function(item) {
            return item.food.GI === 'high'
        })
        var lowGIItems = this.digestionQueue.filter(function(item) {
            return item.food.GI === 'low'
        })
    ...
    MCT1.prototype._makeEffect = function(
        type,
        milliseconds,
        color,
        amplifier
    ...
    MCT1.prototype.extractActivitiesFromMoveLog = function() {
        ...
        this.moveActivityLog.forEach(function(mLog, i) {
            ...
            else if (isUpward) activity = activities_1.activityTypes.JUMP
            else if (mLog.isSprinting)
                activity = activities_1.activityTypes.SPRINT
            else activity = activities_1.activityTypes.WALK
    ...
    MCT1.prototype.ensureInfiniteSnowballs = function() {
        var _this = this
        var itemInSlot = user_1
            .user(this.player)
            .inventory.getItem(this.snowballSlot)
        if (!itemInSlot || !this.isLightningSnowballStack(itemInSlot)) {
            user_1
                .user(this.player)
                .inventory.bumpItemIntoSlot(
                    this.snowballSlot,
    ...
    MCT1.prototype.ensureInfiniteInsulin = function() {
        ...
        var itemInSlot = user_1
            .user(this.player)
            .inventory.getItem(this.insulinSlot)
